...
def import_input(day):
    """
    i just put all the inputs in the same folder for everybody codes
    (this is different to how advent of code is laid out but bc the events are smaller and have way more inputs its not worth it)
    """
    with open(f"day_{str(day)}.txt", "r") as f:
        data = f.read()
    return data
